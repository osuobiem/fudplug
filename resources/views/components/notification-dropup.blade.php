<div class="dropup-container mnd-container animate__animated d-none" onclick="closeMND()">
    <div class="dropup-inner mnd-inner animate__animated">
  
      <ul class="nav nav-justified osahan-line-tab" style="box-shadow: 0 5px 10px 0 #00000061">
        <li class="nav-item text-left">
          <a class="nav-link active dropup-top-a d-flex" style="padding: 0 !important">
            <span class="ml-2 py-2"> <i class="la la-bell la-lg"></i> Notifications</span>
            <div class="text-right">
              <i class="la la-times la-lg dropup-top-ico" onclick="closeComments()"></i>
            </div>
          </a>
        </li>
      </ul>
  
      <div class="pb-2 pt-3">
  
  
      </div>
  
    </div>
  </div>