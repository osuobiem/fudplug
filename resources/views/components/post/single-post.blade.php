...
if(!Auth::guard('vendor')->guest()) {
  ...
  $comment_clickable = isset($from_comms) && $from_comms ? false : true;
@endphp
...
<div class="box shadow-sm border rounded bg-white mb-3 osahan-post" id="post__{{ $post->id }}">
  ...
  <div class="p-3 border-bottom osahan-post-body post-inner">
    <p class="mb-0 f-post" onclick="{{ $comment_clickable ? 'openComments(`'.$post->id.'`, `like`)' : '' }}">{{ $post->content }}</p>
  ...
  <div class="p-3 border-bottom osahan-post-footer">
    ...
    <a onclick="{{ $comment_clickable ? 'openComments(`'.$post->id.'`)' : '' }}" class="mr-3 text-secondary comments-ico-a" title="Comment"><i class="la la-comment la-2x"></i><span class="post-comm-inner-{{ $post->id }}">&nbsp;{{ $post->comments }}</span></a>
